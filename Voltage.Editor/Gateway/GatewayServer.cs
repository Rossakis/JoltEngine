...
public sealed class GatewayServer : IDisposable
{
	...
	public static string InfoFilePath => Path.Combine(EditorStorage.Root, "gateway.json");
	...
	private void ReadLoop(GatewayClient client)
	{
		try
		{
			...
			{
				@event = "hello",
				data = new { editor = "Voltage", pid = Environment.ProcessId, clientId = client.Id }
			}));
	...
	/// <summary>Only authenticated clients keep the editor running unfocused, so an unauthenticated connect changes nothing.</summary>
	private void UpdateKeepAlive()
	{
		lock (_clientsLock)
			Core.KeepRunningWhenUnfocused = _clients.Any(c => c.Authenticated);
	}
	...
	private void WriteInfoFile()
	{
		var info = new
		{
			...
			args = Program.CommandLineArgs ?? Array.Empty<string>(),
			logs = EditorStorage.LogsDirectory
		};
